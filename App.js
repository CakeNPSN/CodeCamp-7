...
class App extends Component {
  ...
  render() {
    return (
      <div className="App">
        <button disabled={this.state.current === 0} onClick={() => { this.setState({ current: this.state.current - 1 }); }}>Previous</button>
        <img style={{ maxWidth: "400px" }} src={picList[this.state.current]} />
        <button disabled={this.state.current === 3} onClick={() => { this.setState({ current: this.state.current + 1 }); }}>Next</button>
      </div >
    );
